...
class LLMService {
  ...
  // 便捷方法：单轮对话
  async chat(systemPrompt: string, userMessage: string, options?: LLMOptions): Promise<string> {
  ...
  // 便捷方法：单轮对话（带使用量统计）
  async chatWithUsage(systemPrompt: string, userMessage: string, options?: LLMOptions): Promise<LLMResponse> {
    const messages: LLMMessage[] = [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: userMessage }
    ];
  ...
  // 便捷方法：JSON 响应
  async chatJSON<T>(systemPrompt: string, userMessage: string, options?: LLMOptions): Promise<T> {
  ...
  // 便捷方法：JSON 响应（带使用量统计）
  async chatJSONWithUsage<T>(systemPrompt: string, userMessage: string, options?: LLMOptions): Promise<{ data: T; usage: TokenUsage; latencyMs: number }> {
  ...
  // 尝试修复截断的 JSON
  private tryFixTruncatedJSON(jsonString: string): string {
