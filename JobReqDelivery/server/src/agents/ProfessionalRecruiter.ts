...
export class ProfessionalRecruiter {
  ...
  async evaluateWithUsage(
    ...
    try {
      const { data: result, usage, latencyMs } = await llmService.chatJSONWithUsage<ProfessionalRecruiterOutput>(
        SYSTEM_PROMPT,
        userPrompt,
