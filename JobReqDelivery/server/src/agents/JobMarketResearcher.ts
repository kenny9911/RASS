...
先识别岗位所在的领域与行业，然后根据领域与行业进行市场研究，识别市场上相似或相关的职位名称，提供行业薪资和经验水平基准，评估人才市场供需状况，构建详细的理想候选人画像。
...
const USER_PROMPT_TEMPLATE = `基于以下职位需求分析结果，请进行对中国大陆就业市场在相同领域的岗位研究：
...
export class JobMarketResearcher {
  ...
  async researchWithUsage(analyzerOutput: RequirementsAnalyzerOutput): Promise<ResearcherResult> {
    ...
    try {
      const { data: result, usage, latencyMs } = await llmService.chatJSONWithUsage<JobMarketResearcherOutput>(
        SYSTEM_PROMPT,
        userPrompt,
