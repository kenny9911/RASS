...
export class AgentOrchestrator {
    ...
    updateTokenUsage(agent, usage, latencyMs) {
        // 更新代理级别的使用量
    ...
    async analyze(requisition, analysisResult) {
        ...
        try {
            while (currentIteration < MAX_ITERATIONS && !isSatisfied) {
                ...
                const recruiterResult = await professionalRecruiter.evaluateWithUsage(requisition, analyzerOutput, researcherOutput, currentIteration);
                ...
                const strategyResult = await recruitingStrategyAgent.validateWithUsage(requisition, analyzerOutput, researcherOutput, recruiterOutput);
                ...
                if (fitScore >= SATISFACTION_THRESHOLD && isApproved) {
                    ...
                    if (currentIteration < MAX_ITERATIONS) {
                        ...
                        this.emit('agent_progress', `适配评分 ${fitScore.toFixed(1)}/10，建议: ${revisions}`, { iteration: currentIteration });
                    }
                }
            }
            // 获取最后一次迭代的结果作为最终结果
