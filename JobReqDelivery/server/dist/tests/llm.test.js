...
describe('OpenRouter Provider', () => {
    ...
    it('should strip openrouter/ prefix from model name', async () => {
        ...
        // 通过私有方法测试（使用反射）
        const getModel = provider.getModel.bind(provider);
        // 模拟配置
...
describe('LLM Service', () => {
    ...
    it('should complete a chat request', async () => {
        const response = await llmService.chat('你是一个简洁的助手', '1+1等于几？只回答数字', { temperature: 0.1, maxTokens: 10 });
        expect(response).toBeDefined();
        expect(response).toContain('2');
        console.log('Chat 响应:', response);
    }, 30000);
    it('should parse JSON response', async () => {
        const response = await llmService.chatJSON('你是一个JSON生成器，只返回JSON格式的数据，不要有任何其他文字', '返回一个JSON对象，包含result字段，值为42。格式：{"result": 42}', { temperature: 0.1, maxTokens: 50 });
        expect(response).toBeDefined();
